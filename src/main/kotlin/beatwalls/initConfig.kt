...
fun initialize(wd: File){
    ...
    val text = exampleMainFile(diff.name, diff.characteristic,hjd,modType)
...
private fun pickDifficulty(info: Info): DiffInfo {
    ...
    for (d in info._difficultyBeatmapSets){
        for(m in d._difficultyBeatmaps){
            diffs.add(DiffInfo(
                m._difficulty,
                m._beatmapFilename,
                d._beatmapCharacteristicName,
            ))
        }
    }

    println("\nChoose the Difficulty you want to work with")
    println("WARING: Beatwalls overwrites the notes, obstacles and events.")
    println("WARING: It is recommended to use a separate file for walls generated by it.\n")

    for ((index, it) in diffs.withIndex()) {
        println("$index -> ${it.name}${it.characteristic}")
    }

    print("\nEnter the Number of the Difficult you want to work with:\ninput: ")
    val index = readLine()?.toIntOrNull()
    if (index == null || index !in 0..diffs.size) {
        println("Lets try that again. No games this time around")
...
private fun exampleMainFile(difficulty: String, characteristic: String,  hjd: Double, Modtype: String): String {
    val s = StringBuilder()
    ...
    s.appendLine("Characteristic: $characteristic")
